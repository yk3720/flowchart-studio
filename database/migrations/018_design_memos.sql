-- v1.1: design memos on devices / units / modules (Excel SSOT + Web editor)
-- Depends on: 003_db2_schema.sql, 016_flow_documents_collaborative_edit.sql
...
comment on column public.devices.memo is
  'Design memo (1 per device). Excel 装置名 sheet · re-import overwrites.';
comment on column public.units.memo is
  'Design memo (1 per unit). Excel ユニット sheet · re-import overwrites.';
comment on column public.modules.memo is
  'Design memo (1 per module). Excel 構成 sheet · re-import overwrites.';

-- import_equipment_bundle — upsert memo on re-import (Excel SSOT)
...
begin
  ...
  v_device_memo := coalesce(trim(p_bundle->>'memo'), '');
  ...
  insert into public.devices (internal_code, display_name, sort_order, memo, created_by)
  values (
    v_code,
    v_name,
  ...
  loop
    insert into public.units (device_id, label, sort_order, memo, created_by)
    values (
      v_device_id,
      v_unit.label,
      coalesce(v_unit.sort_order, 0),
      coalesce(trim(v_unit.memo), ''),
      v_uid
    )
    ...
    loop
      insert into public.modules (unit_id, label, sort_order, memo)
      values (
        v_unit_id,
        v_mod.label,
        coalesce(v_mod.sort_order, 0),
        coalesce(trim(v_mod.memo), '')
      )
...
comment on function public.import_equipment_bundle(jsonb) is
  'Editor/admin bulk import. Upserts device/unit/module memos from bundle (Excel SSOT). '
  'Flow upsert on re-import (collaborative edit).';
