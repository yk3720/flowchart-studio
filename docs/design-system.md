...
| C 内 · 色の階層 | **大面積（ナビ・ペイン背景）= ニュートラル surface** · **彩度の高い `--flow-accent` = プライマリ CTA・小面積選択アクセントのみ**（矢印はレイヤー B）。ナビ全面を CTA と同色 family にしない — [ワークスペースレイアウト方針 §B](./02_機能設計/ワークスペースレイアウト方針.md#b-ナビ配色2026-06-25-改定) |
