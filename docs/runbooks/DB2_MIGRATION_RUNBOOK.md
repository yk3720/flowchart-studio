...
**アプリ確認:** 自分が取込した装置を選択 → その他 → **「装置を削除…」**（M12 · 危険 · §13）
...
## 13. UI — M12 装置削除（§E · N3 廃止）

**2026-06-26 §E 改定パック**（`c92f345`）で UI のみ変更。**DB migration は不要**（010 `rpc_delete_equipment` はそのまま）。

| 項目 | §E 前（N3）                 | §E 後（M12）                                              |
| ---- | --------------------------- | --------------------------------------------------------- |
| 入口 | 左ナビ「装置を削除…」（N3） | 表ペイン **その他 ▼ · 危険** → **「装置を削除…」**（M12） |
| 条件 | `device.canDelete` · editor | 同上                                                      |
| 確認 | D1 ダイアログ               | 同上                                                      |

**N3 はナビから除去済み** — Runbook 内の「ナビ 装置を削除…」は **M12 手順に読み替える**。
...
1. 自分が取込した装置を workspace で選択（`canDelete` が true のもの）
2. 表ペイン **その他 ▼** を開く
3. 危険セクションの **「装置を削除…」** → D1 確認 → 削除成功バナー

**やらない:** ナビに N3 が残っている想定での手順（§E 前の手順書は obsolete）。
