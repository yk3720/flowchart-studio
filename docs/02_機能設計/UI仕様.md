...
**更新:** 2026-06-28（ADR-019 暫定 · ADR-018 レイアウト追随 · doc-sync）  
**目次:** [00\_目次.md](./00_目次.md)  
**関連:** [ADR-009](<../03_技術仕様/意思決定記録(ADR).md>) · [ADR-011](<../03_技術仕様/意思決定記録(ADR).md>) · [ADR-013](<../03_技術仕様/意思決定記録(ADR).md>) · [ADR-018](<../03_技術仕様/意思決定記録(ADR).md#adr-018-3-ペイン入替--右ドック--デモ整備草案--2026-06-27>) · [要求定義書.md](../01_要求定義/要求定義書.md) §4

> **ADR-018（第2弾 · 実装済 · 2026-06-27）:** 以下は **第2弾実装後の現行**（Nav｜**図**｜**表** · 右 `[表|設計メモ|コメント]` · ペイン3内 chrome · 本番列の意味なし · D1）。正本 [ADR-018](<../03_技術仕様/意思決定記録(ADR).md#adr-018-3-ペイン入替--右ドック--デモ整備草案--2026-06-27>) · [grill 右ペイン](../01_要求定義/grill-me_2026-06-27_右ペインと3ペイン整理.md)。残: 設計メモ実機 step2。
...
**ヘッダー常時ボタン（編集者）:** `[再生成]` · `[行を追加]` · **その他 ▼**（JSON 書き出し/読込 · PNG/SVG · 取込 · 雛形 等）— `flowchart-practical-ux-yk` ルール準拠。**再生成はヘッダーのみ**（TablePane 内に同名ボタンは置かない）。
...
┌──────────────────────────────────────────────────────────────────┐
│ AppHeader                                                        │
│  Flowchart Web · [再生成] · [行を追加] · [その他 ▼] · ステータス │  ※2ペイン歴史参照
├─────────────────────────┬────────────────────────────────────────┤
...
| 操作               | 手順                                                                                                                      |
| ------------------ | ------------------------------------------------------------------------------------------------------------------------- |
| **コピー貼り付け** | Excel / スプレッドシートで表範囲をコピー → 点線枠内テキストエリアに貼り付け → **[表に貼り付け]**                          |
| **Excel ファイル** | 同パネル内 **[Excelから取込…]** → `.xlsx` / `.xls` を選択 → **先頭シートを自動読込**（シート手動選択 · 範囲指定は未実装） |
| **反映後**         | 表 UI に行が載る → ヘッダー **[再生成]** でプレビュー更新 · クラウド保存は **再生成時**（workspace モード）               |

**成功時メッセージ:** パネル内に「○ 行を表に貼り付けました。続けて「再生成」してください」と表示。
...
**方針 SSOT:** [ワークスペースレイアウト方針.md](./ワークスペースレイアウト方針.md)（PR-B **実装済** · UX改善パック §D〜§E **実装済**）

| 項目                          | 状態                                                                                                                       |
| ----------------------------- | -------------------------------------------------------------------------------------------------------------------------- |
| ナビ｜表｜図 ドラッグリサイズ | **実装済**（`react-resizable-panels` v4 · PR-B）                                                                           |
| ナビ左揃え（`justify-start`） | **実装済**                                                                                                                 |
| ナビ｜表間余白解消            | **実装済**（`fcNavAside` 全面使用 · [同書 §D](./ワークスペースレイアウト方針.md#d-ナビ表ペイン間の余白2026-06-25-実装済)） |
| 表｜図デフォルト比率 v2       | **実装済**（52:48 · [同書 §E](./ワークスペースレイアウト方針.md#e-デフォルト比率改定--ペイン幅リセット2026-06-25-実装済)） |
| ペイン幅リセット（T4→N9）     | **実装済**（§E · ナビ N9）                                                                                                 |

### 表ペイン UX 改定パック（2026-06-26 · 実装済）

**方針 SSOT:** [表ペインUX方針 §E](./表ペインUX方針.md#e-表ペイン-ux-改定パック2026-06-26--実装済) · ナビ [ワークスペースレイアウト方針 §F](./ワークスペースレイアウト方針.md#f-ナビタイトル--ペイン幅リセット移動2026-06-26-実装済) · ボタン [ボタン一覧.md](./ボタン一覧.md)

| 項目             | 決定                                                       |
| ---------------- | ---------------------------------------------------------- |
| 固定ヘッダー     | 文脈 + 操作ボタン + 列ヘッダー（3 段）                     |
| 行を追加         | H5 · 再生成の右                                            |
| ペイン幅リセット | N9 · ナビ icon（T4 から移動）                              |
| 横スクロールバー | 操作ボタン行の下 · 列ヘッダーの上                          |
| 列幅 auto-fit    | T5 · 横バー行右 icon                                       |
| タイトル         | ナビ「Flowchart Studio」· 実用版バッジ削除 · 「表」h2 削除 |
| 装置を削除       | M12 · その他 · 危険                                        |
| その他メニュー   | `position: fixed`                                          |
| 色列 UI          | 空 = 「-」（保存値は空）                                   |
| select バグ      | 種別/色選択で他列ずれ · **修正済**（`d2603e5`）            |
| 適用             | デスクトップ 3 ペインのみ                                  |

### 表ペイン縦優先（2026-06-25 実装済）

**方針 SSOT:** [表ペインUX方針.md §D](./表ペインUX方針.md#d-表ペイン縦優先レイアウト2026-06-25-実装済)
...
| T4 列幅リセット     | **廃止**                                        |
| T4 ペイン幅リセット | 表ツールバーに新設（**§E で N9 へ移動済**）     |

> **§E 実装済:** 現行コードは [§E 改定パック](#表ペイン-ux-改定パック2026-06-26--実装済) を正とする。
...
| 項目                 | 置き場                                                                          | 備考                                                                        |
| -------------------- | ------------------------------------------------------------------------------- | --------------------------------------------------------------------------- |
| **装置名**           | **NavPane**（`<select>` で装置切替 · **§E:** 先頭に Flowchart Studio タイトル） | ヘッダーには出さない · 永続化キー `${deviceId}:${moduleId}`                 |
| **ユニット名**       | **Pane1 第 1 階層**                                                             | 折りたたみリスト（例：供給・プレス・収納）                                  |
| **動作モジュール**   | **Pane1 第 2 階層**                                                             | 選択単位＝フロー 1 本。クリックで **Pane2（図）/ Pane3（表）** の中身を切替 |
| **フロー図**         | **Pane2（CanvasPane · 中央）**                                                  | React Flow（読取専用）· モジュールごとの最終生成図                          |
| **フロー作成用の表** | **Pane3（TablePane · 右）**                                                     | **10 列表** UI · CSV/JSON · import.json 取込後の微調整                      |

第 4 回 diagram-manager の **3 ペイン**（ナビ｜一覧｜詳細）に相当。**ADR-018 改定後（現行）:** **Nav｜図｜表**（Pane2=Canvas · Pane3=Table · 右 50% · 右ドック `[表|設計メモ|コメント]`）。

### ワイヤ（デスクトップ・ワイド · ADR-018 現行）

```
┌────────────────────────────────────────────────────────────────────┐
│ AppHeader                                                          │
│  文脈（ユニット·動作）· [再生成] · [行を追加] · [その他 ▼]         │
├──────────────┬──────────────────────────────┬──────────────────────┤
│ NavPane      │ CanvasPane                   │ TablePane            │
│ (左)         │ (中央)                       │ (右)                 │
│              │                              │ [表|設計メモ|コメント]│
│ [装置 ▼]     │ 同モジュールの               │ 選択中モジュールの   │
│ ユニット ▼   │ フロー図                     │ 10 列表              │
│  モジュールA │ · stale オーバーレイ         │ · CSV 貼り付け       │
│  モジュールB │                              │                      │
│  …           │ 未選択時: 空状態             │ 未選択時: 空状態     │
└──────────────┴──────────────────────────────┴──────────────────────┘
```

> **Historical（ADR-011 · ADR-018 前）:** Nav｜**表**｜**図**（Pane2=Table · Pane3=Canvas）。PR-B 初期比率 Nav 18% · Table 40% · Canvas 60%。
> **ADR-018 現行比率（実装済 · 2026-06-27）:** Nav **17%** · 図 **33%** · 表 **50%**（**2｜4｜6** · `workspace-inner-v3`）。詳細 [ワークスペースレイアウト方針.md](./ワークスペースレイアウト方針.md)。

**D 要望（2026-05-31）:** フローは縦長のため **CanvasPane を画面上端〜下端まで**使う — **プレビュー chrome 縦レイアウトで対応済**（2026-06-25 · [プレビュー上部chrome方針.md](./プレビュー上部chrome方針.md)）。

### 操作・状態（合意済み）

| 操作                               | 挙動                                                                                              |
| ---------------------------------- | ------------------------------------------------------------------------------------------------- |
| モジュール選択                     | **Pane3** にそのモジュールの表、**Pane2** に**前回生成した図**を表示                              |
| 表を編集して未再生成               | **Pane2** は前回図のまま · stale 等で **「プレビューは古い」**（モジュール単位）                  |
| モジュール切替                     | **モジュールごとに表を自動退避・復元**。保存先 API は未確定 → `repository` で差し替え可能に       |
| ユニットのみ展開・モジュール未選択 | **Pane2/3** は**空状態**（「モジュールを選択してください」）                                      |
| 再生成                             | 選択中モジュールの表 → 生成 → **Pane2** 更新 · 当該モジュールの「最終生成スナップショット」を更新 |
...
| ---------------------- | ------------------------------------------------------------------------------------ |
| ワイド（主）           | 3 ペイン横並び                                                                       |
| 狭い（ノート半画面等） | Pane1: 折りたたみ or ドロワー · **ADR-018 現行:** タブ「**図｜表**」（デフォルト図） |
...
| UI 入口      | **決定（暫定）** — その他 → **「装置を取込…」**（M9 · import.json のみ · [ボタン一覧](./ボタン一覧.md) · [暫定方針](../01_要求定義/grill-me_2026-06-28_装置取込暫定方針.md)） |
| 永続化       | AI スクール「データ保存」授業後に保存先確定。切替時自動退避は**有力**だが binding は後                                                                                        |

### Phase 3 将来拡張

- **複数装置:** **実装済み（2026-05-30）** — Nav 最上段 `<select>`（**現行デモ:** プレス機 A/B · **ADR-018 改定後:** DEMO0000 のみ）。本番 DB 連携は ADR-013
...
**色・テーマ:** テーマ切替は**削除**。枠色は `flowColors.ts` 固定 · ノード背景は表「色」列（**推奨 v2:** index 2 · **v1 読込:** index 9 → v2 へマイグレーション — [ADR-016](<../03_技術仕様/意思決定記録(ADR).md#adr-016-10-列順-v2--3-ペインリサイズ2026-06-25>) PR-A 実装済）— [作者ガイド.md](../03_技術仕様/作者ガイド.md)
...
| 「PNG/SVGをダウンロード」不可 / StatusBar に案内 | その他メニューの書き出しは stale 中ブロック（B-1） |
...
| 警告                              | 琥珀色 · **表の下** · `<details>` デフォルト閉（2026-06-25 実装済）· 生成は継続（ADR-009） | 警告行を確認 · 必要なら修正 → 再生成                                                                                                      |
...
| Excel 装置ファイル一括  | **実装済（暫定）** — その他 →「装置を取込…」· **import.json のみ** · プレビュー確認（ADR-019 暫定） |
| ミニモード · 常時最前面 | なし（Web 対象外）                                                                                  |

---

_ペイン幅のドラッグリサイズは [ワークスペースレイアウト方針.md](./ワークスペースレイアウト方針.md)（PR-B **実装済** · デフォルト比率 v2 · ペイン幅リセット **実装済**）。_
