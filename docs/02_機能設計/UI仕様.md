...
**次:** [ADR-016](<../03_技術仕様/意思決定記録(ADR).md#adr-016-10-列順-v2--3-ペインリサイズ2026-06-25>) — 10列 v2 · ヘッダー短縮 · 列リサイズ修正

### ワークスペースレイアウト（3ペインリサイズ · ナビ配色）

**方針 SSOT:** [ワークスペースレイアウト方針.md](./ワークスペースレイアウト方針.md)（合意済 · 未実装）

| 項目                          | 状態                               |
| ----------------------------- | ---------------------------------- |
| ナビ｜表｜図 ドラッグリサイズ | 未実装（`react-resizable-panels`） |
| ナビ左揃え（`justify-start`） | 未実装                             |
...
**比率目安（現行 · CSS 固定）:** Nav 狭め（〜20%）· Table 40% 前後 · Canvas 40% 前後（`2fr:3fr`）。

**次（未実装）:** [ADR-016](<../03_技術仕様/意思決定記録(ADR).md#adr-016-10-列順-v2--3-ペインリサイズ2026-06-25>) — `react-resizable-panels` でナビ｜表｜図をドラッグリサイズ。詳細は [ワークスペースレイアウト方針.md](./ワークスペースレイアウト方針.md)。

**D 要望（2026-05-31）:** フローは縦長のため **CanvasPane を画面上端〜下端まで**使う — **プレビュー chrome 縦レイアウトで対応済**（2026-06-25 · [プレビュー上部chrome方針.md](./プレビュー上部chrome方針.md)）。横ペイン比率の可変化は ADR-016。
...
_ペイン幅のドラッグリサイズは [ワークスペースレイアウト方針.md](./ワークスペースレイアウト方針.md)（ADR-016 · 未実装）。現状はカスタム CSS 中心（`REACTFLOW_RULES` §0）。_
