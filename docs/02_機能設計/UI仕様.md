...
**更新:** 2026-06-28（ADR-019 暫定 · 装置取込 JSON-only）  
...
第 4 回 diagram-manager の **3 ペイン**（ナビ｜一覧｜詳細）に相当するが、本アプリは **詳細の代わりに「表｜図」の同時表示** を維持する（再生成・stale UX のため）。**ADR-018 改定後**は **Nav｜図｜表**（Pane2=Canvas · Pane3=Table · 右 50%）。
...
**比率目安（PR-B 初期 · Historical）:** Nav **18%** · Table **40%** · Canvas **60%**（内側 Group · ドラッグ可 · `localStorage` 永続化）。**現行デフォルトは下記 §E の 52:48。**

**現行（§E 実装済 · コード）:** Table **52%** · Canvas **48%** · storage キー `-v2`。詳細は [ワークスペースレイアウト方針 §E](./ワークスペースレイアウト方針.md#e-デフォルト比率改定--ペイン幅リセット2026-06-25-実装済)。

**ADR-018 改定後（実装済 · 2026-06-27）:** Nav **17%** · 図 **33%** · 表 **50%**（**2｜4｜6** · `workspace-inner-v3` 想定）。
...
| ---------------------- | ----------------------------------------------------------------------------------------------------------------- |
| ワイド（主）           | 3 ペイン横並び                                                                                                    |
| 狭い（ノート半画面等） | Pane1: 折りたたみ or ドロワー · Pane2｜Pane3: タブ「表｜図」（**現行**）· **ADR-018:** タブ「図｜表」デフォルト図 |
