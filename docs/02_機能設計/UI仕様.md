...
### Web → Excel（計画 · 2026-06-25 grill-me）

**一覧・整理方針:** [ボタン一覧.md](./ボタン一覧.md)（T2/T3 · 未整理）

**場所:** **TablePane** · `FlowTableEditor` の「行を追加」ボタン横（編集者のみ）。

| ボタン                     | クリップボード内容                                                      | 用途                                   |
| -------------------------- | ----------------------------------------------------------------------- | -------------------------------------- |
| **表をコピー**             | いま表示中フローの **データ行のみ**（TSV · **ヘッダーなし**）           | Web 微修正 → 既存 Excel テーブルへ貼付 |
| **列フォーマットをコピー** | **10 列ヘッダー + サンプル 3 行**（端子→処理→端子 · `sample-basic` 系） | 空白シートに表の型を貼る               |

**貼付後（Excel 側）:** 範囲選択 → **挿入 → テーブル** → テーブル名 `{ユニット短名}_{動作名}`（[Excel取込.md §3.3](../03_技術仕様/Excel取込.md)）。

**状態:** 実装済み（2026-06-25 · `copyTableUtils.ts` · `FlowTableEditor` ボタン 2 個）。
