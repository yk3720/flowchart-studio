...
**状態:** スクロール・レイアウト統一は完了（2026-06-25 · `523a008`）。列リサイズの視覚反映・v2 列順は [ADR-016](<../03_技術仕様/意思決定記録(ADR).md#adr-016-10-列順-v2--3-ペインリサイズ2026-06-25>) で未実装。  
...
| Playwright | 取込折りたたみ · バナー非表示 · ドック↔表 scroll 同期 · リセット · 部分貼り付け回帰 · sticky ヘッダー（**列リサイズ視覚反映は ADR-016 で追補**） |
...
**最終更新:** 2026-06-25（実装済 `523a008` · **v2 列順・ヘッダー短縮は ADR-016 参照**）
...
| 項目         | 方針                                                                                                                     |
| ------------ | ------------------------------------------------------------------------------------------------------------------------ |
| schema       | `table-10col-v2` — 色を index 2（図形種別の直後）                                                                        |
| 画面ヘッダー | 短縮ラベル（種別・下先・右先・文1〜3）· `?` 削除                                                                         |
| 列幅         | 接続先 48px · 表 `minWidth` = 列幅合計                                                                                   |
| ペイン       | [ワークスペースレイアウト方針.md](./ワークスペースレイアウト方針.md) — 3ペインリサイズ（ナビ配色は現行ニュートラル維持） |
