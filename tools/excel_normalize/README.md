...
**決定:** 2026-06-19 — 装置ごとの手書き xlsx は **`testdata/devices/`** 以下に置く（`templates/` は空テンプレのみ · `yk-document/` は本アプリの取込 SSOT 外）。
...
| 種別               | パス（リポジトリルートから）                                                    |
| ------------------ | ------------------------------------------------------------------------------- |
| **装置ルート**     | `tools/excel_normalize/testdata/devices/{社内番号}_{装置名}/`                   |
...
**現在の実装置（手書き作業）:** `A0001_塗布装置/` — 一覧は [`testdata/devices/README.md`](testdata/devices/README.md)

```text
c:/yk-application/flowchart-studio/tools/excel_normalize/testdata/devices/A0001_塗布装置/
  A0001_塗布装置.xlsx
  import.json
